...
export const useWithdrawICP = () => {
  ...
  const withdrawICP = async ({
    ...
    if (icp) {
      ...
      if ("Ok" in transfer && transfer.Ok) {
        toast.success(`Transfer Sussessful!`);
        return true;
      } else if ("Err" in transfer && transfer.Err) {
        const errorKey = Object.keys(transfer.Err)[0];
        toast.error(`Transfer failed: ${errorKey}`);
        return false;
      }
    }
    return false;
...
export const useWithdrawIcrc1Tokens = (token_principal?: string) => {
  const [token] = useToken(token_principal);

  const withdrawIcrc1Tokens = async ({
    amount,
    address,
  }: {
    amount: bigint;
