    ...
    <div className="flex h-full w-full flex-col justify-between px-8 py-4">
        ...
        {menuList.map((m: any) => {
          return (
            <Link key={m.name} to={m.link} onClick={close}>
              <div
                className={`my-3.5 flex items-center gap-2 rounded-lg px-5 py-2.5 text-sm font-semibold min-[2560px]:py-4 min-[2560px]:text-4xl ${path === m.link ? "bg-[#f3f3f3] text-black" : "text-[#808080]"
                  }`}
              >
      ...
      <div>
        <div className="relative rounded-lg bg-black px-2 py-3 text-center text-white min-[2560px]:rounded-xl min-[2560px]:px-6 min-[2560px]:py-8">
          ...
          <a
            href="https://discord.com/invite/WQG7hjNqv7"
            target="_blank"
