...
const Home = () => {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<PaginatedCampaigns>(`/api/campaigns?${queryParams}`);
      ...
      if (isConnected && principal) {
        if (rewardToken && tokenCanister && platformfee && platformfee[1]) {
          const userConfirmed = window.confirm(
            ...
            `We will also transfer ${platformfee[0]
        ...
        {["In-Progress", "Upcoming", "Pending"].includes(
          selectedOption as string,
            ...
            <td className="p-4 font-bold min-[2560px]:text-2xl">
              <CountdownTimer date={String(Time)} isTimeUnix callback={() => {
        ...
        {["Unclaimed"].includes(selectedOption as string) && (
          <td className="p-4">
            <Button
              ...
              className="w-fit cursor-pointer hover:bg-gray-600 rounded-md bg-black1 px-4 py-2 text-xs text-white min-[2560px]:text-3xl"
            >
              <p>Claim</p>
            </Button>
          </td>
    ...
    <AuthLayout>
      <div className="relative">
        ...
        <div className="min-h-screen bg-gray1 md:p-4">
          ...
          <div
            className={`mt-4 sm:mt-7 flex flex-col lg:flex-row gap-y-3 gap-x-6`}
          >
            {["Ended"].includes(selectedOption as string) && (
              <>
          ...
          <div
            className={`mt-8 overflow-x-auto rounded-lg border border-gray2 ${["Claimed", "Unclaimed"].includes(selectedOption as string)
          ...
          <div
            className={`mt-4 flex flex-col justify-between lg:flex-row ${["Claimed", "Unclaimed"].includes(selectedOption as string)
              ? "lg:mt-4"
              : "lg:mt-0"
