import plugLogoLight from "./plugLight.svg";
import plugLogoDark from "./plugDark.svg";
...
class PlugWallet implements IConnector, IWalletConnector {
  ...
  async init() {
    ...
    try {
      if (!this.#ic) {
        ...
        if (!window.ic?.plug) return err({ kind: InitError.NotInstalled });
        this.#ic = window.ic?.plug;
      }
      if (!this.#ic) {
        console.log("window.ic?.plug 2", window.ic?.plug);
  ...
  async connect() {
    try {
      if (!this.#ic) {
        if (!window.ic?.plug) {
          console.log("window.ic?.plug connect", window.ic?.plug);
      ...
      if (!this.#ic) {
        console.log("window.ic?.plug connect 2", window.ic?.plug);
        window.open("https://plugwallet.ooo/", "_blank");
        return err({ kind: ConnectError.NotInstalled });
      }
