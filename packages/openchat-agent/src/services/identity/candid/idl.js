...
const idlFactory = ({ IDL }) => {
    ...
    const CreateIdentityArgs = IDL.Record({
        ...
        'challenge_attempt': IDL.Opt(IDL.Record({ 'key': IDL.Nat32, 'chars': IDL.Text })),
    });
    ...
    return IDL.Service({
        'approve_identity_link': IDL.Func([ApproveIdentityLinkArgs], [ApproveIdentityLinkResponse], []),
        'auth_principals': IDL.Func([IDL.Record({})], [AuthPrincipalsResponse], ['query']),
        'check_auth_principal': IDL.Func([IDL.Record({})], [CheckAuthPrincipalResponse], ['query']),
        'create_identity': IDL.Func([CreateIdentityArgs], [CreateIdentityResponse], []),
        'generate_challenge': IDL.Func([IDL.Record({})], [GenerateChallengeResponse], []),
        'get_delegation': IDL.Func([GetDelegationArgs], [GetDelegationResponse], ['query']),
        'initiate_identity_link': IDL.Func([InitiateIdentityLinkArgs], [InitiateIdentityLinkResponse], []),
