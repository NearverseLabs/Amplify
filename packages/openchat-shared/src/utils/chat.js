...
exports.mapCancelP2PSwapResponseToStatus = exports.mapAcceptP2PSwapResponseToStatus = exports.contentTypeToPermission = exports.chatIdentifierToString = exports.routeForChatIdentifier = exports.routeForMessageContext = exports.routeForMessage = exports.compareRoles = exports.emptyRules = exports.emptyChatMetrics = exports.compareChats = exports.getDisplayDate = exports.getMinVisibleEventIndex = exports.indexRangeForChat = exports.getContentAsText = exports.getContentAsFormattedText = exports.userIdsFromEvents = exports.userIdsFromTransactions = void 0;
const domain_1 = require("../domain");
function userIdsFromTransactions(transactions) {
    return transactions.reduce((userIds, t) => {
        if (t.from !== undefined) {
            userIds.add(t.from);
        }
        if (t.to !== undefined) {
...
exports.userIdsFromTransactions = userIdsFromTransactions;
function userIdsFromEvents(events) {
    const fakeFormatter = (k) => k;
    return events.reduce((userIds, e) => {
        ...
        switch (e.event.kind) {
            case "message":
                ...
                if (e.event.repliesTo !== undefined &&
                    e.event.repliesTo.kind === "rehydrated_reply_context") {
                    userIds.add(e.event.repliesTo.senderId);
                    (0, domain_1.extractUserIdsFromMentions)(getContentAsFormattedText(fakeFormatter, e.event.repliesTo.content, {})).forEach((id) => userIds.add(id));
                }
                ...
                (0, domain_1.extractUserIdsFromMentions)(getContentAsFormattedText(fakeFormatter, e.event.content, {})).forEach((id) => userIds.add(id));
                break;
            case "member_joined":
            case "member_left":
...
exports.userIdsFromEvents = userIdsFromEvents;
function getContentAsFormattedText(formatter, content, cryptoLookup) {
    let text;
    if (content.kind === "text_content") {
        ...
        text = captionedContent(formatter("tokenTransfer.transfer", {
        ...
        throw new domain_1.UnsupportedValueError("Unrecognised content type", content);
    }
    return text.trim();
}
exports.getContentAsFormattedText = getContentAsFormattedText;
...
function captionedContent(type, caption) {
    if (caption) {
        return type + " - " + caption;
    }
    else {
...
function indexRangeForChat(chat) {
    return [getMinVisibleEventIndex(chat), chat.latestEventIndex];
}
exports.indexRangeForChat = indexRangeForChat;
function getMinVisibleEventIndex(chat) {
    if (chat.kind === "direct_chat")
        return 0;
    return chat.minVisibleEventIndex;
}
exports.getMinVisibleEventIndex = getMinVisibleEventIndex;
function getDisplayDate(chat) {
...
exports.compareChats = compareChats;
...
function routeForMessage(scope, ctx, messageIndex) {
...
exports.routeForMessage = routeForMessage;
function routeForMessageContext(scope, { chatId, threadRootMessageIndex }, open = false) {
    return threadRootMessageIndex === undefined
        ? routeForChatIdentifier(scope, chatId)
        : `${routeForChatIdentifier(scope, chatId)}/${threadRootMessageIndex}${open ? "?open=true" : ""}`;
}
exports.routeForMessageContext = routeForMessageContext;
function routeForChatIdentifier(scope, id) {
...
exports.routeForChatIdentifier = routeForChatIdentifier;
function chatIdentifierToString(chatId) {
    switch (chatId.kind) {
        case "direct_chat":
        ...
        default:
            throw new domain_1.UnsupportedValueError("Unknown chatId kind", chatId);
    }
}
exports.chatIdentifierToString = chatIdentifierToString;
function contentTypeToPermission(contentType) {
    switch (contentType) {
        case "image_content":
            return "image";
        case "video_content":
        ...
        case "file_content":
            return "file";
        default:
            throw new domain_1.UnsupportedValueError("Unknown attachment content type", contentType);
    }
}
exports.contentTypeToPermission = contentTypeToPermission;
function mapAcceptP2PSwapResponseToStatus(response, userId) {
...
exports.mapAcceptP2PSwapResponseToStatus = mapAcceptP2PSwapResponseToStatus;
function mapCancelP2PSwapResponseToStatus(response) {
    switch (response.kind) {
        case "success":
            return {
                kind: "p2p_swap_cancelled",
...
exports.mapCancelP2PSwapResponseToStatus = mapCancelP2PSwapResponseToStatus;
